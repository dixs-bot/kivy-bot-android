            ...
            if settings["platform_facebook"]:
                for attempt in range(settings["retry_max"]):
                    ...
                    if attempt < settings["retry_max"] - 1:
                        log_store.add("warning", f"Retry FB ({attempt+2}/{settings['retry_max']})", "facebook")
            ...
            if settings["platform_instagram"]:
                for attempt in range(settings["retry_max"]):
                    ...
                    if success:
                        ...
                        break
                    if attempt < settings["retry_max"] - 1:
                        log_store.add("warning", f"Retry IG ({attempt+2}/{settings['retry_max']})", "instagram")
                        await asyncio.sleep(3)
                else:
                    self.total_failed += 1
            ...
            if settings["platform_whatsapp"]:
                for attempt in range(settings["retry_max"]):
                    ...
                    if success:
                        ...
                        break
                    if attempt < settings["retry_max"] - 1:
                        log_store.add("warning", f"Retry WA ({attempt+2}/{settings['retry_max']})", "whatsapp")
                        await asyncio.sleep(3)
                else:
                    self.total_failed += 1
...
_settings_store: Dict = {
    ...
    "platform_facebook": True, "platform_instagram": True, "platform_whatsapp": True,
    "retry_max": 3, "api_key_fb": "", "api_key_ig": "", "api_key_wa": ""
}
...
def _apply_schedule():
    ...
    day_map = {
        "senin": schedule_lib.every().monday, "selasa": schedule_lib.every().tuesday,
        "rabu": schedule_lib.every().wednesday, "kamis": schedule_lib.every().thursday,
        "jumat": schedule_lib.every().friday, "sabtu": schedule_lib.every().saturday,
