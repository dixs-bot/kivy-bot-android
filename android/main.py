...
# ============================================================
C = {
    'bg':           (0.024, 0.043, 0.094, 1),
    'bg_card':      (0.067, 0.102, 0.180, 1),
    'bg_card_h':    (0.086, 0.125, 0.251, 1),
    'border':       (0.102, 0.153, 0.267, 1),
    'border_l':     (0.141, 0.200, 0.337, 1),
    'accent':       (0.000, 0.898, 0.627, 1),
    'accent_dim':   (0.102, 0.239, 0.200, 1),
    'accent_glow':  (0.200, 1.000, 0.761, 1),
    'error':        (1.000, 0.278, 0.341, 1),
    'error_dim':    (0.165, 0.051, 0.078, 1),
    'warning':      (1.000, 0.647, 0.008, 1),
    'warning_dim':  (0.165, 0.102, 0.008, 1),
    'info':         (0.561, 0.643, 0.788, 1),
    'info_dim':     (0.051, 0.102, 0.165, 1),
    'text':         (0.910, 0.929, 0.961, 1),
    'muted':        (0.420, 0.478, 0.600, 1),
    'white':        (1, 1, 1, 1),
    'fb':           (0.357, 0.498, 0.843, 1),
    'ig':           (0.910, 0.376, 0.541, 1),
    'wa':           (0.145, 0.820, 0.400, 1),
...
class DataStore:
    ...
    def add_account(self, label, platform="all"):
...
class UploadScreen(Screen):
    ...
    def show_file_chooser(self):
        ...
        def on_select(instance, selection, *args):
            if selection:
                ...
                if fp.endswith('.csv'):
                    self._filepath = fp
                    self.file_label = 'Terpilih: ' + os.path.basename(fp)
                    self.has_file = True
    ...
    def _parse_local(self, filepath):
        self._local_rows = []
        try:
    ...
    def _render_preview(self):
        ...
        for i, p in enumerate(self._local_rows[:20]):
            row = ProductRow(
                nama=p.get('nama', '-'),
                harga_str='Rp ' + p.get("harga", "0"),
        ...
        if len(self._local_rows) > 20:
            lbl = Label(
                text='... dan ' + str(len(self._local_rows) - 20) + ' produk lainnya',
                font_size='11sp',
                color=C['muted'],
                size_hint_y=None,
                height='20dp',
    ...
    def _upload_ok(self, msg, count):
        self.upload_result = 'Berhasil: ' + msg
        self.result_color = C['accent']
        toast_mgr.show(str(count) + ' produk berhasil diupload', 'success')
    ...
    def _upload_fail(self, detail):
        self.upload_result = 'Gagal: ' + detail[:60]
        self.result_color = C['error']
...
class MoreMenuScreen(Screen):
    ...
    def on_enter(self):
        ...
        self.connection_info = api.base_url + ' | ' + mode
...
class AccountScreen(Screen):
        ...
        for acc in store.accounts:
            ...
            def make_bind(a):
                def on_touch(instance, touch):
                    if instance.collide_point(*touch.pos):
                        ...
                        self._render_accounts()
                        toast_mgr.show(
                            'Akun "' + a["label"] + '" ' + ('diaktifkan' if a["active"] else 'dinonaktifkan'),
                            'info'
                        )
                        return True
                    return False
            ...
            def make_long_bind(a, r):
                def on_long(instance, touch):
                    ...
                    self._render_accounts()
                    toast_mgr.show('Akun "' + a["label"] + '" dihapus', 'warning')
                return on_long
    ...
    def show_add_dialog(self):
        ...
        def do_add(instance):
            ...
            self._render_accounts()
            popup.dismiss()
            toast_mgr.show('Akun "' + name + '" ditambahkan', 'success')
